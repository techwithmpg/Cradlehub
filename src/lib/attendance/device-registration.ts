...
async function resolveCrmSignals(
  request: StaffDeviceRegistrationRequest,
...
export async function getOwnAttendancePhoneState(
  rawCredential: string | null
): Promise<StaffAttendancePhoneState | null> {
  ...
  const requestQuery = supabase
    ...
    .select(
      "id, staff_id, branch_id, request_type, status, device_label, browser_name, platform_name, existing_device_id, replacement_device_id, completed_device_id, requested_at, reviewed_at, reviewer_note, rejection_reason, expires_at"
    )
  ...
  return {
    ...
    registeredDevice: current
      ? { id: current.id, label: current.label, lastSeenAt: current.lastSeenAt }
      : null,
...
export async function requestAttendancePhoneRegistration(params: {
  ...
  if (inserted.error || !inserted.data)
    throw new Error(inserted.error?.message ?? "Request could not be created.");
...
export async function completeOwnAttendancePhoneRequest(params: {
  ...
  if (!row?.success || !row.device_id) {
    throw new Error(
      row?.code === "approval_expired"
        ? "This approval expired. Submit a new request."
        : "This approval cannot be activated on this phone."
    );
  }
...
export async function renameOwnAttendanceDevice(deviceId: string, label: string): Promise<void> {
  ...
  if (trimmed.length < 2 || trimmed.length > 60)
    throw new Error("Use a phone name between 2 and 60 characters.");
  const admin = asAttendanceDb(createAdminClient());
  const updated = await admin
    .from("staff_devices")
...
export async function listStaffDeviceRegistrationRequests(
  branchId: string
): Promise<StaffDeviceRegistrationRequest[]> {
  const admin = asAttendanceDb(createAdminClient());
  const result = await admin
    .from("staff_device_registration_requests")
    .select(
      "id, staff_id, branch_id, request_type, status, device_label, browser_name, platform_name, existing_device_id, replacement_device_id, completed_device_id, requested_at, reviewed_at, reviewer_note, rejection_reason, expires_at, staff:staff!staff_device_registration_requests_staff_id_fkey(full_name)"
    )
...
async function getRegistrationRequestById(
  requestId: string
): Promise<StaffDeviceRegistrationRequest | null> {
  const admin = asAttendanceDb(createAdminClient());
  const result = await admin
    .from("staff_device_registration_requests")
    .select(
...
export async function reviewStaffDeviceRegistrationRequest(params: {
  ...
  const rpc = await admin.rpc("review_staff_device_registration_request", {
    ...
    p_token_hash:
      params.decision === "approved" ? hashRecoveryToken(createRecoveryToken()) : undefined,
  ...
  await Promise.all([
    ...
    createStaffNotification({
      ...
      title:
        params.decision === "approved"
          ? "Attendance phone approved"
          : "Attendance phone request declined",
      body:
        params.decision === "approved"
...
export async function reconcileFirstScanDeviceRegistration(params: {
  ...
  const result = await admin
    .from("staff_device_registration_requests")
    .select(
      "id, staff_id, branch_id, request_type, status, device_label, browser_name, platform_name, existing_device_id, replacement_device_id, completed_device_id, requested_at, reviewed_at, reviewer_note, rejection_reason, expires_at, activation_token_id"
    )
  ...
  if (
    result.data.request_type === "replacement" &&
    replacementDeviceId &&
    replacementDeviceId !== params.deviceId
  ) {
  ...
  if (result.data.activation_token_id) {
    await admin
      ...
      .eq("id", result.data.activation_token_id)
      .is("used_at", null);
  }
  await admin
    .from("staff_device_registration_requests")
...
function rootAttendanceScanRequestId(operationId: string | null | undefined): string | null {
  const value = operationId?.trim();
  if (!value) return null;
  return value.replace(/:register(?::.*)?$/, "");
}

export async function resolveFirstScanUnknownDeviceIncident(params: {
  branchId: string;
  staffId: string;
  deviceId: string;
  ...
  try {
    ...
    const scan = await admin
      ...
      .eq("reason_code", "unknown_device")
      .maybeSingle();

    if (scan.error || !scan.data?.id) return;
    ...
    const incidents = await admin
      ...
      .eq("branch_id", params.branchId)
      .eq("status", "open")
      .eq("exception_type", "unknown_device")
      .or(`scan_event_id.eq.${scan.data.id},latest_scan_event_id.eq.${scan.data.id}`);

    if (incidents.error || !incidents.data?.length) return;

    const now = new Date().toISOString();
    const incidentIds = incidents.data.map((row) => row.id);
    const resolved = await admin
      .from("attendance_exceptions")
      .update({
        staff_id: params.staffId,
        status: "resolved",
        resolution_owner: "staff",
        resolution_status: "resolved",
        staff_response_required: false,
        resolution_action: "staff_signed_in_and_connected_phone",
        resolution_note:
          "Staff signed in on the scanned phone and completed secure device connection.",
        resolved_at: now,
      })
      .in("id", incidentIds)
      .eq("status", "open");
    ...
    await Promise.all(
      incidentIds.map((incidentId) =>
        resolveNotificationsForEntity("attendance_exception", incidentId, "crm")
      )
    );
    ...
    await createStaffNotification({
      ...
      type: "attendance_device_registration_completed",
      title: "Attendance phone connected",
      body: "This phone is connected and the earlier phone issue was closed automatically.",
      entityType: "attendance_exception",
      entityId: incidentIds[0],
      actionHref: "/staff-portal/attendance",
      priority: "normal",
    });
  } catch {
    // Incident cleanup is best-effort. A cleanup problem must never block the
    // successful phone connection or the Attendance scan that follows it.
  }
}
