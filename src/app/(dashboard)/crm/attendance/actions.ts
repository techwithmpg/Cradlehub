...
export type AttendanceActionResult =
  ...
  | {
      ok: true;
      kind: "issue_message" | "technical_escalation";
      tab: "exceptions";
      message: string;
      exceptionId: string;
    }
...
export async function refreshCrmAttendanceWorkspaceAction(): Promise<
  | { ok: true; data: Awaited<ReturnType<typeof getAttendanceWorkspaceData>> }
  | { ok: false; error: string }
> {
  ...
  try {
    ...
    return { ok: false, error: safeError(error, "Attendance could not be refreshed.") };
  }
}

export async function loadAttendanceHistoryAction(input: {
  fromDate: string;
  toDate: string;
}): Promise<
  | { ok: true; data: Awaited<ReturnType<typeof getAttendanceHistoryData>> }
  | { ok: false; error: string }
> {
  const datePattern = /^\d{4}-\d{2}-\d{2}$/;
  if (!datePattern.test(input.fromDate) || !datePattern.test(input.toDate)) {
  ...
  if (days < 0 || days > 366) return { ok: false, error: "Choose a range of up to one year." };
  const ctx = await getAttendanceActionContext({});
  if (!ctx) return { ok: false, error: "Attendance access is no longer available." };
  try {
    return { ok: true, data: await getAttendanceHistoryData({ branchId: ctx.branchId, ...input }) };
  } catch (error) {
    return { ok: false, error: safeError(error, "Attendance history could not be loaded.") };
  }
}
...
export async function ensureRoomQrPointsAction(): Promise<AttendanceActionResult> {
  ...
  try {
    ...
    return {
      ok: false,
      tab: "qr",
      message: safeError(error, "Could not generate missing QR codes."),
    };
  }
}

export async function replaceAttendanceQrAction(
  qrPointId: string
): Promise<AttendanceActionResult> {
  const context = await getContextOrResult("qr");
  if ("result" in context) return context.result;
  if (!qrPointId) return { ok: false, tab: "qr", message: "QR code ID is missing." };
  try {
    const point = await replaceBranchAttendanceQrPoint({ ctx: context.ctx, qrPointId });
    revalidateAttendanceSurfaces();
    return {
      ok: true,
      kind: "attendance_qr",
      ...
      qrPoint: await hydrateQrPoint(point, await getOrigin()),
    };
  } catch (error) {
    return { ok: false, tab: "qr", message: safeError(error, "Could not replace the QR code.") };
  }
}

export async function createDeviceActivationTokenAction(
  formData: FormData
): Promise<AttendanceActionResult> {
  ...
  try {
    ...
    return {
      ok: false,
      tab: "devices",
      message: safeError(error, "Could not activate the phone."),
    };
  }
}

export async function revokeAttendanceDeviceAction(
  formData: FormData
): Promise<AttendanceActionResult> {
  ...
  try {
    ...
    return {
      ok: false,
      tab: "devices",
      message: safeError(error, "The selected device is no longer active."),
    };
  }
}
...
export async function resolveAttendanceExceptionAction(
  ...
  try {
    ...
    return {
      ok: false,
      tab: "exceptions",
      message: safeError(error, "Could not resolve the exception."),
    };
  }
}
...
export async function reviewAttendanceExceptionAction(
  ...
  try {
    ...
    return {
      ok: false,
      tab: "exceptions",
      message: safeError(error, "Could not mark the issue reviewed."),
    };
  }
}
...
export async function askStaffAboutAttendanceIssueAction(input: {
  ...
  if (!message || message.length > 1000)
    return { ok: false, tab: "exceptions", message: "Enter a question up to 1,000 characters." };
  const admin = createAdminClient();
  const issue = await admin
    .from("attendance_exceptions")
    .select("id, branch_id, staff_id, exception_type")
    .eq("id", input.exceptionId)
    .eq("branch_id", context.ctx.branchId)
    .eq("status", "open")
    .maybeSingle();
  if (issue.error || !issue.data?.staff_id)
    return {
      ok: false,
      tab: "exceptions",
      message: "This open issue is not linked to a staff member.",
    };
  const now = new Date().toISOString();
  const messages = admin.from("attendance_issue_messages" as never) as unknown as {
    insert: (value: Record<string, unknown>) => Promise<{ error: { message: string } | null }>;
  };
  const inserted = await messages.insert({
    exception_id: issue.data.id,
    branch_id: issue.data.branch_id,
    staff_id: issue.data.staff_id,
    sender_staff_id: context.ctx.actorStaffId,
    sender_workspace: "crm",
    message,
    response_choices: input.responseChoices ?? [],
    created_at: now,
  });
  if (inserted.error)
    return { ok: false, tab: "exceptions", message: "Could not save the staff question." };
  await admin
    .from("attendance_exceptions")
    .update({ resolution_status: "waiting_for_staff", staff_response_required: true })
    .eq("id", issue.data.id);
  await createOrUpdateNotification({
    ...
    type: "attendance_issue_question",
    title: "Attendance question from CRM",
    body: message,
    ...
    priority: "high",
    requiresAction: true,
    dedupeKey: `attendance-question:${issue.data.id}`,
    metadata: {
      exceptionId: issue.data.id,
      responseChoices: input.responseChoices ?? [],
      scanIssueType: issue.data.exception_type,
    },
  });
  await createOrUpdateWorkflowTask({
    branchId: issue.data.branch_id,
    workspaceScope: "crm",
    taskType: "attendance_issue_follow_up",
    title: "Waiting for staff Attendance response",
    body: message,
    entityType: "attendance_exception",
    entityId: issue.data.id,
    actionHref: "/crm/attendance?tab=exceptions",
    priority: "high",
    dedupeKey: `attendance-follow-up:${issue.data.id}`,
  });
  revalidateAttendanceSurfaces();
  return {
    ok: true,
    kind: "issue_message",
    tab: "exceptions",
    message: "Question sent to the Staff Portal inbox.",
    exceptionId: issue.data.id,
  };
}

export async function escalateAttendanceIssueAction(
  exceptionId: string
): Promise<AttendanceActionResult> {
  const context = await getContextOrResult("exceptions");
  if ("result" in context) return context.result;
  ...
  const issue = await admin
    .from("attendance_exceptions")
    .select("id, branch_id, safe_error_code, exception_type")
    .eq("id", exceptionId)
    .eq("branch_id", context.ctx.branchId)
    .eq("status", "open")
    .maybeSingle();
  if (issue.error || !issue.data)
    return { ok: false, tab: "exceptions", message: "Open Attendance issue not found." };
  await admin
    .from("attendance_exceptions")
    .update({
      resolution_status: "waiting_for_technical_support",
      resolution_owner: "technical_support",
    })
    .eq("id", exceptionId);
  await createOrUpdateWorkflowTask({
    branchId: issue.data.branch_id,
    ...
    taskType: "attendance_technical_escalation",
    title: "Attendance technical escalation",
    body: `Safe code: ${issue.data.safe_error_code ?? issue.data.exception_type}`,
    entityType: "attendance_exception",
    entityId: issue.data.id,
    actionHref: "/owner/attendance?tab=exceptions",
    priority: "critical",
    dedupeKey: `attendance-technical:${issue.data.id}`,
  });
  revalidateAttendanceSurfaces();
  return {
    ok: true,
    kind: "technical_escalation",
    tab: "exceptions",
...
export async function applyAttendanceCorrectionAction(
  ...
  try {
    ...
    return {
      ...
      tab: "exceptions",
      message: ["allow_branch_today", "change_permanent_branch"].includes(input.actionType)
        ? "Branch corrected. Ask the staff member to scan again."
        : result.message,
    };
  } catch (error) {
    return {
      ok: false,
      tab: "exceptions",
      message: safeError(error, "Could not apply the attendance correction."),
    };
  }
}
...
export async function updateAttendanceRulesAction(
  ...
  try {
    ...
    return {
      ok: false,
      tab: "exceptions",
      message: safeError(error, "Could not save attendance rules."),
    };
  }
}
...
export async function completeDueServiceSessionsAction(): Promise<AttendanceActionResult> {
  ...
  try {
    ...
    return {
      ok: false,
      tab: "sessions",
      message: safeError(error, "Due sessions could not be completed."),
    };
  }
}
