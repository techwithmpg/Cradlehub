...
const resolveBranchCorrectionSchema = z
  .object({
    requestId: z.guid("Invalid request ID"),
    decisionType: z.enum([
  ...
  .superRefine((value, context) => {
    if (value.decisionType === "permanent_branch_transfer" && !value.reason?.trim()) {
      context.addIssue({
        code: "custom",
        path: ["reason"],
        message: "Enter a short transfer reason.",
      });
    }
  });
...
const resolveBranchAssignmentIssueSchema = z
  .object({
  ...
  .superRefine((value, context) => {
    ...
    if (value.resolutionType === "correct_permanent_primary_branch" && !value.effectiveDate) {
      context.addIssue({
        code: "custom",
        path: ["effectiveDate"],
        message: "Choose the transfer effective date.",
      });
    }
    if (value.resolutionType === "grant_temporary_branch_access") {
      if (!value.validFrom || !value.validUntil || !value.temporaryScope) {
        context.addIssue({
          code: "custom",
          path: ["validUntil"],
          message: "Temporary access requires a bounded validity period.",
        });
        return;
      }
      if (
        !Number.isFinite(Date.parse(value.validFrom)) ||
        !Number.isFinite(Date.parse(value.validUntil))
      ) {
        context.addIssue({
          code: "custom",
          path: ["validUntil"],
          message: "Temporary access dates are invalid.",
        });
      }
    }
  });
...
export async function getBranchCorrectionImpactAction(
  rawInput: unknown
...
export async function resolveBranchCorrectionRequestAction(
  ...
  if (!actor) {
    return {
      ...
      message: "CRM access is required to resolve branch corrections.",
    };
  }
...
export async function rejectBranchCorrectionRequestAction(
  ...
  if (!actor) {
    return {
      ok: false,
      code: "UNAUTHENTICATED",
      message: "CRM access is required to reject branch corrections.",
    };
  }
...
export async function resolveBranchAssignmentIssueAction(
  ...
  const defaultReason =
    input.resolutionType === "confirm_wrong_qr_scan"
