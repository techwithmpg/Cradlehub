...
export default async function StaffTodayPage() {
  ...
  const driverItems =
    driverResult && !("error" in driverResult)
      ? driverResult.items
      : [];
  ...
  return (
    <>
      {attendanceData ? (
        <div className={isTherapist ? "hidden md:block" : undefined}>
          <StaffAttendanceSummary data={attendanceData} />
        </div>
      ) : null}
      ...
      {attendanceData ? (
        <StaffAttendanceRealtime staffId={attendanceData.staffId} />
      ) : null}
    </>
  );
}