...
export default async function OnboardPage({
    ...
    return (
      <div style={{ maxWidth: 480, margin: "4rem auto", textAlign: "center", padding: "0 1rem" }}>
        ...
        <p style={{ fontSize: "0.875rem", color: "var(--cs-text-muted)", lineHeight: 1.6 }}>
          This invite has already been used. Your account is pending approval from the owner or
          manager.
        </p>
      </div>
    );
  ...
  // Staff already activated — invite no longer valid
  if (staff.is_active) {
    return (
      <div style={{ maxWidth: 480, margin: "4rem auto", textAlign: "center", padding: "0 1rem" }}>
        <div style={{ fontSize: "3rem", marginBottom: "1rem" }}>🔒</div>
  ...
  if (isExpired) {
    return (
      <div style={{ maxWidth: 480, margin: "4rem auto", textAlign: "center", padding: "0 1rem" }}>
        ...
        <p style={{ fontSize: "0.875rem", color: "var(--cs-text-muted)", lineHeight: 1.6 }}>
          This invite link expired on {expiry.toLocaleDateString("en-PH")}. Please ask your
          administrator to generate a new one.
        </p>
      </div>
    );
  }

  const branchName = staff.branches?.name ?? "your branch";
