...
export function SelectedBookingCommandPane({
  ...
  return (
    <aside className="sticky top-4 flex h-[calc(100vh-78px)] min-h-[690px] max-h-[940px] min-w-[560px] flex-col overflow-hidden rounded-xl border border-[var(--cs-border-soft)] bg-white shadow-[var(--cs-shadow-sm)]">
      ...
      <div className="mt-4">
        ...
        <SelectedBookingPrimaryAction booking={effectiveBooking} dispatchHref={dispatchHref} onOpenArrival={onOpenArrival} onOpenRoom={onOpenRoom} onSessionStarted={(startedAt) => setSessionOverride({ status: "in_progress", booking_progress_status: "session_started", session_started_at: startedAt })} onChanged={onChanged} />
      </div>

      <div className="mt-3">
        <SelectedBookingQuickActions booking={effectiveBooking} viewerRole={viewerRole} statusAction={statusAction} onOpenReschedule={onOpenReschedule} onChanged={onChanged} />
      </div>
