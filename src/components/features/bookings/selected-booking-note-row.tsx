...
export function SelectedBookingNoteRow({
  ...
  return (
    <SelectedBookingOverviewRow
      ...
      action={canEdit ? <button type="button" onClick={() => { setDraft(savedNote); setEditing(!editing); }} className={overviewActionClass}>{editing ? "Close" : savedNote ? "Edit" : "Add note"}</button> : undefined}
    >
      {editing ? (
        <div className="grid gap-2">
          ...
          <div className="flex justify-end gap-2">
            <button type="button" onClick={() => setEditing(false)} className={overviewActionClass}>Cancel</button>
