...
function sessionState(session: AttendanceWorkspaceData["sessions"][number]): string {
              ...
              {activeSessions.map((session) => (
                <tr key={session.id} className="border-b last:border-b-0">
                  ...
                  <td className="px-3 py-3">
                    <RemainingProgress
                      remainingMinutes={remainingMinutes(session.session_due_at)}
                      totalMinutes={session.duration_minutes ?? 60}
                    />
                  </td>
                  <td className="px-3 py-3">
                    <StatusPill value={sessionState(session)} />
