...
export function AttendanceScanFeedRow({
  ...
  selectedDate: string;
  onSelect?: (scan: RecentAttendanceScan) => void;
}) {
  ...
  const content = (
    <>
      <span
        ...
        style={
          scan.staffAvatarUrl ? { backgroundImage: `url("${scan.staffAvatarUrl}")` } : undefined
        }
        aria-hidden="true"
      >
      ...
      <span className="min-w-0">
        <span className="block truncate font-bold text-[var(--cs-text)]">{displayName}</span>
  ...
  if (onSelect) {
    return (
      <button
        ...
        className="grid min-h-14 w-full grid-cols-[2rem_minmax(0,1fr)_auto] items-center gap-3 rounded-md px-1 py-2 text-left text-sm transition hover:bg-[#f7eddd] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#b78a42]/35"
        aria-label={`Open ${scan.staffName} attendance details`}
      >
        {content}
      </button>
    );
