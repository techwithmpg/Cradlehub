...
function DriverBottomNav() {
  ...
  const items = [
    ...
    { label: "Route",    href: "/driver/route",       icon: Route },
...
function CurrentTripCard({ trip }: { trip: DriverTrip }) {
  ...
  return (
    ...
    >
      ...
      {trip.hs_address && (
        <div style={{ fontSize: 12, color: "var(--cs-text-muted)", display: "flex", alignItems: "center", gap: 5 }}>
          <MapPin size={12} />
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {trip.hs_address && (
          <div style={{ fontSize: 11, color: "var(--cs-text-muted)", marginTop: 3, display: "flex", alignItems: "center", gap: 4 }}>
            <MapPin size={10} />
            <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {trip.hs_address}
            </span>
...
export function DriverMobileHome({ driver, trips }: DriverMobileHomeProps) {
  ...
  const upcomingTrips  = sorted.filter((t) => t.booking_progress_status === "not_started");
  const completedTrips = sorted.filter((t) => t.booking_progress_status === "completed");
  ...
  return (
    <div style={{ minHeight: "100dvh", backgroundColor: "var(--cs-bg)", paddingBottom: 96 }}>
      ...
      >
        <div>
          <div style={{ fontSize: 16, fontWeight: 800, color: "var(--cs-text)", letterSpacing: "-0.01em", lineHeight: 1 }}>
            CradleHub
          </div>
          <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#5A7A8A", marginTop: 1 }}>
            Driver Portal
          </div>
        </div>
      ...
      >
        ...
        >
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "0.5rem" }}>
            <div>
              <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: "var(--cs-text)", lineHeight: 1.2 }}>
        ...
        {allDone && (
          ...
          >
            ...
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "var(--cs-text)" }}>All trips completed!</div>
              <div style={{ fontSize: 12, color: "var(--cs-text-muted)", marginTop: 2 }}>Great work today.</div>
            </div>
          </div>
        )}
        ...
        {upcomingTrips.length > 0 && (
          ...
          >
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--cs-text-muted)" }}>
              Upcoming Trips
            </div>
            {upcomingTrips.map((t) => (
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "0.625rem" }}>
            <Truck size={16} color="var(--cs-sand-dark)" />
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: "var(--cs-text)" }}>Dispatch Console</div>
              <div style={{ fontSize: 11, color: "var(--cs-text-muted)" }}>Full trip management and routing</div>
            </div>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "0.5rem" }}>
          {[
            ...
            <button
              ...
              onClick={() => alert(`${label}: Please contact your manager.`)}
            >
              {label}
            </button>
          ))}
