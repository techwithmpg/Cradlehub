...
type LinkRow = { kind: "link"; label: string; description: string; href: string; icon: React.ElementType; disabled?: boolean };
type LogoutRow = { kind: "logout"; label: string; description: string; icon: React.ElementType };
...
function IconBox({ icon: Icon, danger = false }: { icon: React.ElementType; danger?: boolean }) {
  return (
    <div style={{ width: 38, height: 38, borderRadius: 10, backgroundColor: danger ? "rgba(239,68,68,0.08)" : "var(--cs-surface-warm)", border: `1px solid ${danger ? "rgba(239,68,68,0.15)" : "var(--cs-border-soft)"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
...
function MenuRow({ item }: { item: MenuRowItem }) {
  if (item.kind === "logout") {
    return (
      <form action={driverLogoutAction} style={{ width: "100%" }}>
  ...
  if (item.disabled) {
    return <div style={{ opacity: 0.4, cursor: "not-allowed" }} aria-disabled="true"><RowContent item={item} showChevron={false} /></div>;
  }
  return <Link href={item.href} style={{ textDecoration: "none", display: "block" }}><RowContent item={item} /></Link>;
}

function SectionCard({ section }: { section: MenuSection }) {
  return (
    <div>
      <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--cs-text-muted)", marginBottom: "0.375rem" }}>{section.title}</div>
