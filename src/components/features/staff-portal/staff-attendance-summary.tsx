...
export function StaffAttendanceSummary({ data }: { data: StaffAttendanceData }) {
  ...
  return (
    <section
      ...
      aria-labelledby="my-attendance-summary-title"
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {data.scheduleLabel} · {data.todayState.displayLabel}
          </p>
        </div>
        <Badge
          variant={data.currentClockState === "clocked_in" ? "default" : "secondary"}
          className="gap-1"
        >
          {data.currentClockState === "clocked_in" ? (
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm">
        <span>
          <span className="text-muted-foreground">Latest in:</span>{" "}
          {record
        ...
        <span>
          <span className="text-muted-foreground">Latest out:</span>{" "}
          {record?.checkedOutAt
        ...
        <span
          className={record?.reviewState === "review" ? "text-amber-700" : "text-muted-foreground"}
        >
          {record?.reviewLabel ??
            (data.scheduleState === "day_off" ? "No review needed" : "No attendance recorded yet")}
        </span>
        <Link
