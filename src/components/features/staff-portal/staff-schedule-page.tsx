...
import { StaffMobileBottomNav } from "./mobile/staff-mobile-bottom-nav";
...
function GridDayCol({
  ...
  return (
    ...
    >
      {GRID_HOURS.map((h, i) => (
        <div key={h} className={styles.gridHourLine} style={{ top: i * HOUR_HEIGHT }} />
      ))}
      {!isOff && visibleEvents.map((evt) => <GridEvent key={evt.id} event={evt} />)}
    </div>
  );
}
