...
export function buildAvailabilityReadinessIssues(
  ...
  if (summary.notCheckedIn > 0) {
    ...
    issues.push({
      ...
      fix: "Check in arrived staff from the Daily Timeline, or mark unavailable staff as absent.",
  ...
  if (summary.driversTotal > 0 && summary.driversReady === 0) {
    issues.push({
      ...
      fix: "Check in available drivers from the Daily Timeline.",
      actionLabel: "Open Daily Timeline",
      actionHref: "/crm/schedule",
      source: "CrmAvailabilitySummary",
...
export function buildServiceStaffNoScheduleIssue(count: number): ReadinessIssue {
  ...
  return {
    ...
    fix: "Go to Schedule Setup and add an individual weekly pattern.",
    actionLabel: "Open Schedule Setup",
    actionHref: "/crm/schedule?tab=setup",
