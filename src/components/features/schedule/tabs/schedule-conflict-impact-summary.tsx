import { AlertOctagon, CheckCircle2, ClipboardCheck, Info, Wrench } from "lucide-react";
import type {
  ScheduleConflictImpactCounts,
  ScheduleConflictImpactGroup,
  ScheduleConflictTabKey,
} from "./schedule-conflict-center-model";
import { formatIssueCount, getImpactGroupCopy } from "./schedule-conflict-center-model";
...
export function ScheduleConflictImpactSummary({
  counts,
  activeTab,
  onTabChange,
}: Props) {
  const totalActive =
    counts.must_fix + counts.needs_approval + counts.cleanup_warning + counts.informational;

  if (totalActive === 0 && counts.accepted === 0) {
    return (
      <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950 shadow-sm">
        ...
        <p className="mt-2 text-xs leading-5 text-emerald-900">
          No active schedule issues were found for this schedule window.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2">
      {impactOrder.map(({ group, tab, icon: Icon, className }) => {
        ...
        return (
          <button
            ...
            className={`w-full rounded-2xl border p-3 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md motion-reduce:hover:translate-y-0 ${className} ${
              active ? "ring-2 ring-emerald-800/20" : ""
            }`}
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex min-w-0 items-start gap-2">
                <span className="mt-0.5 inline-flex size-7 shrink-0 items-center justify-center rounded-full bg-white/75">
