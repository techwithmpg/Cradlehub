...
- /owner/_ uses an Owner workspace guard; /manager/_ routes still redirect to /crm via layout.tsx
...
### DEC-EXACT-CRM-TIME-001: CRM exact-time start policy

**Date:** 2026-07-24
**Status:** ACCEPTED
**Decision:** For CRM Walk-in, Phone, Future, and Home Service bookings, the therapist must be scheduled at the requested service start. The service may finish after the shift. The full service and Home Service travel range still participate in conflict checks.
**Safety:** Cross-midnight booking remains blocked until date-aware overnight booking storage is implemented.

---

### ATTENDANCE-DIAGNOSTICS-PREVENTION-001: Fix, prevent, verify

**Date:** 2026-07-24
**Status:** ACCEPTED
**Decision:** Every Attendance incident must identify the exact problem, protect the record, provide one clear staff instruction and one clear CRM repair, record the root cause and prevention owner, verify the prevention, and monitor recurrence.
**Incident rule:** One open incident per staff, business date, branch, root category, and affected Attendance record. Repeated scans update occurrence evidence instead of creating duplicate queue rows.
