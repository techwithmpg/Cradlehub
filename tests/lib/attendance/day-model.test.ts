...
function record(overrides: Partial<AttendanceRecord> = {}): AttendanceRecord {
  return {
    ...
    shift_type: "single",
    scheduled_start_at: "2026-07-14T01:00:00.000Z",
    scheduled_end_at: "2026-07-14T10:00:00.000Z",
    checked_in_at: "2026-07-14T01:00:00.000Z",
    checked_out_at: null,
    status: "checked_in",
  ...
  return resolveAttendanceDayStaffStates({
    ...
    schedules: new Map([
      [
        "staff-1",
        params.resolved ??
          schedule([{ shiftType: "single", startTime: "09:00", endTime: "18:00" }]),
      ],
    ]),
...
describe("authoritative attendance day model", () => {
  ...
  it("keeps a missed first split-shift window late during the later gap", () => {
    const state = resolve({
    ...
    expect(state.operationalStatus).toBe("missing");
  });
  ...
  it("resolves an overnight window against its business date", () => {
    const state = resolve({
      ...
      resolved: schedule([
        { shiftType: "closing", startTime: "22:00", endTime: "02:00", endsNextDay: true },
      ]),
    });
  ...
  it("uses a date override supplied by the canonical resolver", () => {
    const state = resolve({
      resolved: schedule(
        [{ shiftType: "single", startTime: "09:00", endTime: "18:00" }],
        "override"
      ),
    });
  ...
  it("classifies an approved day off as not expected", () => {
    ...
    expect(state).toMatchObject({
      scheduleState: "day_off",
      currentAttendanceState: "not_expected",
      operationalStatus: "not_expected",
      displayLabel: "Day Off",
    });
  });
  ...
  it("does not mark a later shift as not arrived", () => {
    ...
    expect(state).toMatchObject({
      scheduleState: "scheduled_later",
      currentAttendanceState: "not_expected",
    });
  });

  it("marks a missing arrival late only after grace expires", () => {
    expect(resolve({ now: "2026-07-14T01:11:00.000Z" })).toMatchObject({
      currentAttendanceState: "late_not_arrived",
      operationalStatus: "missing",
    });
  });

  it("keeps a scheduled no-show late after the shift ends", () => {
    expect(resolve({ now: "2026-07-14T12:00:00.000Z" })).toMatchObject({
  ...
  it("uses the same composite open-close coverage as the scan engine", () => {
    const resolved = schedule([
      { id: "opening", shiftType: "opening", startTime: "10:00", endTime: "17:00", windowOrder: 1 },
      {
        id: "closing",
        shiftType: "closing",
        startTime: "17:00",
    ...
    resolved.coverageKind = "open_close";
    const state = resolve({ now: "2026-07-14T02:48:00.000Z", resolved });

    expect(state.shiftWindows).toHaveLength(1);
    expect(state.currentShiftWindow).toMatchObject({
      id: "closing",
      startTime: "10:00",
      endTime: "01:30",
      scheduledStartAt: "2026-07-14T02:00:00.000Z",
      scheduledEndAt: "2026-07-14T17:30:00.000Z",
    });
  });

  it("reports a checked-in staff member as operationally available", () => {
    expect(resolve({ records: [record()] })).toMatchObject({
      currentAttendanceState: "available",
  ...
  it("reports a completed attendance record as clocked out", () => {
    const state = resolve({
      records: [
        record({
          status: "checked_out",
          checked_out_at: "2026-07-14T10:00:00.000Z",
          worked_minutes: 540,
        }),
      ],
    });
  ...
  it("gives active service precedence over generic availability", () => {
    const session: AttendanceSession = {
      ...
      session_started_at: "2026-07-14T02:00:00.000Z",
      session_due_at: null,
      session_completed_at: null,
      duration_minutes: 60,
    };
    expect(resolve({ records: [record()], sessions: [session] })).toMatchObject({
      currentAttendanceState: "in_service",
      operationalStatus: "on_service",
      availabilityState: "in_service",
      ...
      exceptions: [
        {
          ...
          scan_event_id: null,
          staff_name: "Maria Santos",
          exception_type: "schedule_mismatch",
          severity: "warning",
          status: "open",
          message: "Review",
          metadata: {},
          detected_at: "2026-07-14T01:00:00.000Z",
          resolved_at: null,
          resolved_by: null,
  ...
  it("surfaces a first closing scan without attendance as scan captured", () => {
    const state = resolve({
      exceptions: [
        {
          id: "exception-captured",
          branch_id: "branch-1",
          staff_id: "staff-1",
          checkin_id: null,
          scan_event_id: "scan-1",
          staff_name: "Maria Santos",
          exception_type: "likely_closing_scan_without_clock_in",
          severity: "warning",
          status: "open",
          message: "Captured",
          metadata: {},
          detected_at: "2026-07-14T10:00:00.000Z",
          resolved_at: null,
          resolved_by: null,
          resolved_by_name: null,
          resolution_note: null,
