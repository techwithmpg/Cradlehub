...
describe("Attendance scan resolution", () => {
  it.each([
    ["wrong_branch", "branch", true],
    ["missing_schedule", "schedule", true],
    ["schedule_conflict", "schedule", true],
    ["off_day_scan", "schedule", true],
    ["device_limit_reached", "device", true],
    ["device_not_registered", "device", false],
    ["DEVICE_REVOKED", "security", true],
    ["active_service", "booking_or_service", false],
    ["duplicate_scan", "duplicate", false],
    ["ATTENDANCE_RLS_DENIED", "technical", true],
    ["ATTENDANCE_RPC_MISSING", "technical", true],
    ["unmapped_failure", "technical", true],
  ...
  it("does not create an incident for a harmless committed duplicate", () => {
    ...
    expect(resolution.title).toBe("Scan already received");
