...
2. **Universal Media Picker Component (`UniversalMediaPicker`):**
   ...
   - Integrated as primary interaction for `imageUrl` and `secondaryImageUrl` fields in both Digital Marketer Workspace (`/marketing`) and Owner Marketing Studio (`/owner/marketing`) without discarding unsaved parent form state.
3. **Usage Analyzer & Archive Safety Engine (`media-usage-analyzer.ts`):**
   ...
   - **Explicit UNKNOWN Handling:** Non-standard consumers (custom CSS urls, inline rich-text markup, uncataloged third-party embeds) remain classified as unknown/unproven usages rather than falsely claimed safe.
...
4. **Upload & Storage Safety:**
   - Server-side file upload directly to `public-site-media` bucket with maximum file size verification (<= 10MB), MIME type validation (`image/jpeg`, `image/png`, `image/webp`, `image/gif`, `image/svg+xml`), filename sanitization, and DB insertion enforcing `alt_text >= 3` characters.
...
- `src/lib/validations/marketing.ts`
  ...
  - `marketingMediaAssetInputSchema`: Enforces `altText >= 3` characters, `bucketPath` regex pattern `^[a-z0-9][a-z0-9_./-]*$`.
...
- `src/lib/queries/marketing-media.ts`
  ...
  - `updateMarketingMediaAssetStatus(data, staffId, userRole)`: Enforces role boundaries (marketer: `draft` <-> `submitted`; owner: `approved`, `published`, `archived`).
...
- **Touch Target Standard:** All interactive buttons and picker trigger controls meet the 44px minimum touch target height requirement (`h-11` or min-h-[44px], with 38px-44px compact utility buttons padded for tap comfort).
...
- **Responsive Viewport QA:**
  ...
  - `1024px` / `1280px+` (Desktop): Auto-fill responsive grid (`minmax(140px, 1fr)`), fixed right-hand Asset Inspector drawer (420px width).
...
- **Rollback:** `git revert` of the C5 Pass 2 commit cleanly restores the previous state without orphan records or broken dependencies.
