...
- **Driver split:** The dedicated Driver workspace covers `/driver/*`, but `driver-more-menu.tsx` hardcodes links to `/staff-portal/profile`, attendance, jobs, and map. Primary `driver` role is excluded from the `staff_portal` workspace, creating an immediate navigation/redirect conflict.
- **Utility split:** The Utility role maps to the `/utility/*` workspace (currently a Coming Soon placeholder). The page provides a "Back to Staff Portal" navigation link, but the primary `utility` role is excluded from `staff_portal`, causing a redirect loop or access denial.
...
- **Direct-route concerns:** Direct URL manipulation of proxy routes (`/driver/*`, `/utility/*`, `/staff-portal/*`) is intercepted by layout/page authorization checks. However, inconsistent redirects between `/driver` and `/staff-portal` risk confusing users.
