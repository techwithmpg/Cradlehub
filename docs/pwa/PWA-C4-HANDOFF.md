...
The design incorporates owner-provided visual references across General/CRM, Utility, Therapist, and Driver modes (header branding/avatar/bell layout, rounded card structure, scanner viewfinder with white corner brackets and green scanline, circular checkmark confirmation badges, and Driver map layout) while strictly reconciling and enforcing C3 boundaries:
- **CRM vs Staff install identity contract**: Freezes distinct visible app names ("CradleHub Staff" for Team Workspace vs "CradleHub CRM" for Management Workspace), short names, launch contexts, icon/badge treatments, and install messaging ("Install CradleHub Staff" vs "Install CradleHub CRM"). Staff surfaces do not advertise CRM, and CRM surfaces do not advertise Staff.
- **Manifest and service-worker architecture deferred to C5**: C4 specifies UI/UX contracts and does NOT invent manifest IDs, scopes, `start_url` values, or worker scopes. PWA-C5 must inspect existing CRM architecture and choose a safe Staff manifest/scope including `/driver` without breaking the CRM PWA. `public/sw.js` and `public/cradlehub-push-sw.js` are preserved.
- **Corrected C3 decision references**: Restored canonical traceability to PWA-C3-D019 for CRM/general surfaces and PWA-C3-D013 for Utility boundary.
- **Driver visual references did NOT authorize route, ETA or live-sync claims**: Road route polylines, ETA, route distance, and vehicle travel animations are removed from the baseline C4 Driver map and explicitly categorized under **DEFERRED / CONDITIONAL LATER CAPABILITY**. The misleading copy “Live synced to CRM” is prohibited; baseline Driver design uses truthful snapshot/location states (“Location shared”, “Snapshot sent”, “Location updated [time]”, “Latest location received”).
- **Utility Work remains BLOCKED**: Source inspection confirmed no authoritative Work task backend exists; Utility Work opens an explanatory unavailable page (C4-S19) under Q009. Mockup task checklists do not authorize a task backend in V1.
- **CRM/General personal work only**: Checklist items in visual references do not authorize task-management systems under PWA-C3-D019.
