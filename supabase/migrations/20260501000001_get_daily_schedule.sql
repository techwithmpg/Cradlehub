-- Migration: get_daily_schedule RPC
-- Purpose: Return the full daily schedule for one branch and one date.
-- This is a read-only additive feature. It does not modify booking logic,
-- auto-assignment, or any existing scheduling engine.
...
comment on function public.get_daily_schedule(uuid, date)
  is 'Returns daily schedule rows for a branch and date. Read-only.';
