-- CRADLE-INDIVIDUAL-SCHEDULING-SIMPLIFICATION-005
-- Runtime schedule resolution is individual-only. Group schedule tables may
-- remain as dormant/template data, but booking availability and the Daily
-- Timeline must not inherit group rules when a staff member has no individual
-- weekly schedule.
...
COMMENT ON FUNCTION public.get_available_slots(UUID, UUID, UUID, DATE) IS
  'Availability engine with pending-payment hold support and overnight schedule-window support. Precedence: override day-off/times > individual staff schedule > no schedule.';
...
COMMENT ON FUNCTION public.get_daily_schedule(UUID, DATE) IS
  'Daily board data with overnight schedule-window support. Work span is min start and max absolute end across override/individual windows only.';
